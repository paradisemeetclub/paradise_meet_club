    ...
    $.extend(Plugin.prototype, {
        ...
        initSlideshow: function() {
            ...
            var $slideshow = $('<div class="zs-slideshow"></div>'),
                ...
                $navWrap = $('<div class="zs-navigation container"></div>'),
                $zslayer = $('<div class="zs-layer"></div>'),
                $bulletsWrap = $('<div class="zs-bullets"></div>');

            $('<div class="zs-bullet-count zs-bullet-first">01</div>').appendTo($bulletsWrap);
            ...
            for (i = 0; i < this.numSlides; i++) {
                ...
                if (this.settings.titles.length) {

                    var $nav = $('<div class="zs-nav zs-nav-' + i + '"><span class="lte-image" style="background-image: url(' + this.settings.titles[i]['image'] + ')"></span><span class="lte-header">' + this.settings.titles[i]['title'] + '</span></div>')

                    $('<span class="lte-zs-cursor"></span>').appendTo($navWrap);
                    $nav.appendTo($navWrap);
                }
            ...
            $('<div class="zs-bullet-count zs-bullet-last">' + ('0' + this.numSlides).slice(-2) + '</div>').appendTo($bulletsWrap);
            ...
            $slideshow.append($navWrap).prependTo(this.$el);
            //			$slideshow.after( $navWrap );

            if (this.settings.bullets == true || this.settings.bullets == 'outside') {
            ...
            this.$el.on('click', '.zs-nav', function(e) {
                self.next();
            });
            ...
            this.$nav = $navWrap.children('.zs-nav');
        ...
        jump: function(pos) {
            ...
            if (typeof this.$nav !== "undefined") {
                ...
                this.$nav.eq(this.pos).removeClass('active');
                this.$nav.eq(pos).addClass('active');
            }
