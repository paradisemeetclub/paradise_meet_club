...
function initEvents() {
    ...
    if (jQuery('.lte-hover-3d').length) {

        jQuery('.lte-hover-3d').on('mousemove', function(e) {

            var cardItem = this.querySelector(".lte-hover-3d img");
            var force = 5;
            var offsetY = -(e.offsetY - cardItem.offsetHeight / 2) / force;
            var offsetX = (e.offsetX - cardItem.offsetWidth / 2) / force;
            cardItem.style.transform = 'rotateX(' + offsetY + 'deg) rotateY(' + offsetX + 'deg)';
        });

        jQuery('.lte-hover-3d').on('mouseleave', function(e) {

            jQuery('.lte-hover-3d img')[0].style.transform = 'rotateX(0) rotateY(0)';
        });
    ...
    jQuery('.lte-events-sc.lte-events-layout-grid .lte-item')
        .on('mouseover', function(i, el) {
            ...
            jQuery(this).find('.lte-event-pre').css({
                '-webkit-transform': 'translateY(-' + height + 'px)',
                'transform': 'translateY(-' + height + 'px)'
            });
        })
    ...
    jQuery('.lte-cursor-follow-wrapper').mousemove(function(i, el) {
        ...
        jQuery(this).find('.lte-tab-cursor').css({
            "left": relX,
            "top": relY
        });
    });

    jQuery('.lte-cursor-follow-parent').mousemove(function(i, el) {
...
function initSearch() {
    ...
    jQuery('#lte-top-search-ico-mobile, #lte-top-search-ico-only').on('click', function() {
...
function setResizeStyles() {
    ...
    jQuery('.lte-services-sc.lte-layout-tabs').each(function(i, el) {
        ...
        if (!wrapper.hasClass('hasHeight')) {
            ...
            wrapper.find('.lte-items').css('height', wrapper.find('.lte-items').outerHeight());
